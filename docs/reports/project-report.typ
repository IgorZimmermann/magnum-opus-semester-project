...
= Abstract
This report addresses the administrative burden faced by outpatient departments,
where Danish GPs consult an average of 49 patients per day while spending 10-20 hours per week on administrative tasks.
To tackle this, a privacy-preserving outpatient department management system was developed,
keeping all sensitive patient data within the clinic's network.
The motivation is to modernize clinical workflows using open-source AI tools,
improve consultation quality through AI-assisted second opinions,
...
with secure authentication and a consistent deployment environment across all services.

The finished system covers the full consultation workflow, with separate access for doctors and patients,
AI-generated summaries and prescriptions that doctors can review and adjust,
and automatic prescription delivery to patients by email.

By the end of the semester, a working and secure outpatient consultation system had been built,
putting into practice a service-based architecture alongside locally hosted AI.
The system can be extended further, for example by incorporating patient history into AI suggestions,
processing multiple audio recordings simultaneously,
or expanding the AI's role in supporting diagnosis.
...
The project applies skills gathered from this semester's subjects to provide a working solution for managing consultations in clinical environments,
...
where components are connected through well-defined interfaces for an easily maintainable system.
Additionally, to ensure better data security and privacy,
the AI models are hosted locally to guarantee that no patient data ever leaves the clinic's network.
...
shorter consultation time due to lack of time and doctors losing work hours on paperwork.
#footnote[
  Beskrivelse af almen praksissektoren i Danmark (2016)
]

To address these problems, AI-based tools can be used to ease the workload by automating the creation of prescriptions,
managing doctor's appointments and providing AI-suggested second opinions.
Given the sensitivity of medical records / health data and the strict data protection requirements,
all of these tools are locally hosted, ensuring patient data never leaves the clinic's network.
This was a priority throughout the design and implementation of this project.


== Objective
The project's aim is to deliver a working OPD management system that supports doctors' everyday workflows.
The system's design provides doctors with a user-friendly interface to record and manage consultations
and easily create prescriptions along with AI suggested medical advice.


The booking system serves as a proof of concept to simulate how a full patient-to-doctor workflow would look like in a production like environment.
This also provides the system with the patient's email address needed to send the doctors note to complete the full consultation workflow.
...
Doctors begin and conclude consultations from their portal.
The system records the audio and passes it to a local speech-to-text service for transcription.

*Summary Generation:*
A locally hosted LLM, served through Ollama,
processes the transcript and produces a structured consultation summary to cut down irrelevant non-medical transcript.
...
= Methodology
Before starting out on our project, we outlined a set of rules and methods that ensured that
progression throughout the project would be smooth and continuous.
The methodology and tools used ensured that
every contribution made to the project was peer-reviewed which added accountability to each task.

== Task tracking
In order to ensure all of the tasks are accounted for,
we made every task (programming, diagramming, documenting) an issue on Jira.
...
On Mondays, we held meetings, where we reflected on the Sprint ending that day and
planned the one coming up by assigning tasks from the backlog.
On Thursdays, we held stand-up meetings,
where everyone gave an update on their issue(s) to track each others progress,
and to find bottlenecks as soon as possible.

To ensure that tasks were distributed fairly, we assigned tasks not based on their sheer quantity,
but on actual difficulty.
We agreed collectively on an issue's story point value using Story Point Poker,
see example @sprint-4-backlog.
...
we have also decided to store them in the same repository as our documents and code.
We have created a meeting log document during each meeting to keep everyone accountable and
to allow team members to catch up in case they were absent.

== Repository
To ensure code contributions are safe, each issue had its own branch,
and pull requests had to be opened.
Those pull requests had to be reviewed by at least two non-contributing members.
In order to allow for a rigorous and in-depth review of each contribution,
we made it a rule that pull requests must be opened by Fridays,
which left us the entire weekend for review and refactoring.

To support reviewers and to guarantee a smooth workflow,
a Pull Request Template has also been made, see @pr-template.
We merged all pull requests together during our Monday meetings,
so we could resolve possible merge conflicts with all contributors' inputs.
We have created a pipeline that sends a message to our Discord server about a new pull request,
and by replying to that message, the PR owner had to tag the requested reviewers.
...
All in all, these methods and rules ensured consistent progress across all sprints and
an even distribution of work with no missed deadlines, compared to previous semesters.
With constant peer reviewing, our codebase remained consistent and maintained quality.
...
however they are often cloud based.
...
Operating at this quantity and volume leaves little room for error,
...
The aim of this project is to provide a privacy-preserving,
...
so that patient data never leaves the hospital's network.

To achieve this, the system is broken down into multiple independent services:
appointment management, transcription, clinical summarisation,
medical suggestions, and prescription generation.
These services are isolated, each with its own runtime environment and well-defined interfaces that
connect them and enable easy replacement and seamless upgrades of individual components without affecting the rest of the system.
...
As this system handles sensitive EU patient health data,
security was a priority throughout the design and implementation of this project.
To reduce the risk of data breaches,
all AI models are locally hosted ensuring no patient data is sent to external servers.

Structured booking data is stored in a relational database, while sensitive clinical data
(transcripts, summaries, and prescriptions) is stored in a separate non-relational database.
Role-based access control and
token-based authentication prevent unauthorised access at the backend level,
with doctors and patients having separate authentication tenants and access scopes.
Data in transit is encrypted in non-development environments to further protect sensitive information.


=== Componentised Architecture

A tightly coupled system would make individual failures spread across the entire application.
By breaking the system into loosely coupled components with clearly defined interfaces,
failures are isolated to the affected component while the rest of the system continues to function.
For example, if the LLM service goes down,
only the summary and prescription generation is affected while the rest of the system continues to operate.
This also simplifies debugging and makes individual components independently replaceable without affecting others.

Docker containerisation is used to enforce isolation between services.
Each component runs in its own container, which also enables independent scaling and
automatic health checks.
Containers that fail to respond can be automatically restarted,
meaning the system recovers from individual failures without any manual intervention,
directly addressing the reliability requirements of the system.
...
This semester's project was designed with a component-based system (CBS) design approach focusing on modularity and reusability,
communicating through well-defined interfaces via RESTful APIs, where components are independently deployed,
enabling fault isolation and easy replaceability.

A layered architecture enforces vertical separation of concerns across frontend, backend, and databases.
...
For transcription, Faster-Whisper came out as the most reliable in testing,
with high transcription accuracy and acceptable speed,
...
for the structured booking data. The second one is a non-relational database, MongoDB in our case,
for patient health data documents.
...
During the design phase, the most important aspect for the databases were ACID properties, speed and ease-of-use with the built-in object-relational mapping.
...
MongoDB stores the documents that we create via the LLM.
These are large and semi-structured, allowing us to keep AI outputs from the relational database,
while still being able to easily query them by appointment ids.
The collections are `consultations`, `raw_transcripts`, `summaries`, and `doctor_notes`.
These are responsible for transcripts, summaries and the generated doctor's note.

For data persistence, Docker mounts volumes, so data is saved even after a restart.
This allows us to look back at past records and possibly fine-tune the agents prompt to match our standards.
...
To make the naming conventions clear in the system architecture,
we assigned each component a distinct name to make service boundaries easier to identify.
...
== Containerization
Docker and Docker Compose were utilized to containerize each component to keep the environment consistent.
A single `docker-compose.yml` at the repository root is responsible for the orchestration of the entire system.
...
Database credentials are read from an `.env` file.
Service-to-service URLs are injected as environment variables at the container level,
using Docker Compose's internal DNS to resolve service names (e.g., `http://odin:11434`).
...
so the stack recovers automatically from individual container failures with no need of manual intervention.

== Service-by-Service Implementation

=== Speech-to-Text Service (Echo)
A single processing endpoint `POST /process` accepts an audio file in the request and returns a JSON response.
The uploaded audio is written to a temporary file and is deleted immediately after transcription.
...
Since `/root/.ollama` is mounted to the `ollama_data` volume,
the model is persisted and only needs to be pulled once.
Odin is called from Heimdall via the `POST /api/chat` endpoint,
which accepts a JSON request body containing the model and input messages and returns the generated response.

=== PDF Generation Service (Saga)
`POST /generate` accepts a JSON body with data of the doctor, patient, diagnosis, description, and prescription.
With that information, a JSON file buffer is created.
Afterwards, the data is passed against the note template and a PDF file is created.
The service reads the file into memory, sends it back as an HTTP response, and deletes the file so it doesn't persist.
When Heimdall calls the service, the file gets attached to the email sent via Hermes.

=== Email Service (Hermes)
Hermes is the service that sends the generated doctor's note to the patients as well as booking confirmations.
It uses the `axllent/mailpit` Docker image.
`POST /api/v1/send` requests a PDF from the Typst service,
then sends it to Hermes as an SMTP message with the PDF attached.

=== Booking Backend (Janus)
This backend has two controllers:
- `AppointmentController`
  - `POST /api/appointment` creates a booking and triggers a confirmation email via Hermes.
  - `GET /api/appointment` returns the user's appointments (see @appointment_controller).
...
- `TranscriptController`
  - `POST /api/transcript/GenerateTranscript` accepts the audio file,
    forwards it to Echo and stores the result in `raw_transcripts`.
...
- `PrescriptionController`
  - `POST /api/prescription/GeneratePrescription` sends the summary to Odin again and
    stores the prescription and suggestions in `summaries`. (see @generate_prescription)
...
Iris' booking page displays the patient's existing bookings and
allows creating a new one through a form with doctor selection and a date/time picker.
(`iris/app/booking/page.tsx`) (see @booking_1 and @booking_2)

=== Consultation Frontend (Eir)
There are four pages in this frontend:

Firstly, the dashboard shows the authentication prompt and all appointments for the day, before and after logging in respectively.
(`eir/app/page.tsx`) (see @consultation_1)

Secondly, the doctor can begin the consultation.
(`eir/app/appointment/[id]/page.tsx`) (see @consultation_2)

Thirdly, the generated transcript is shown and ready for review.
(`appointment/[id]/transcript/page.tsx`)

Lastly, the prescription is up for editing and approval.
(`appointment/[id]/note/page.tsx`)
...
in the `Infrastructure/` layer of the consultation backend. This acts as an abstraction for a translation
layer between the business logic and the external service APIs.

== Authentication & Authorization
Both backends use Auth0 JWT Bearer authentication,
configured in Program.cs via `AddAuth0ApiAuthentication` with domain and audience read from `appsettings.json`.
...
The project employs a carefully selected technology stack for quality assurance and
validation across multiple components.

=== Consultation Backend - ASP.NET

As both backends are built using ASP.NET, we utilized xUnit as the primary testing framework,
which is essential for testing asynchronous operations in the backend.
Moq provides flexible mocking capabilities, allowing isolation of external dependencies
(MongoDB, HTTP services, email services) during testing.
This combination enables comprehensive service-level testing
without requiring live instances of MongoDB or third-party APIs.

*Consultation Service Tests*

What is tested:
- The ability to retrieve consultation data and create new consultations.
- The ability to preserve all relevant metadata when creating a consultation record in MongoDB and
  assign a unique consultation ID.
- The ability to retrieve consultation data and map it to the API response format,
  ensuring data integrity across relational and non-relational storage layers.

*Transcript Service Tests*

What is tested:
- The ability to upload audio recordings to the speech-to-text service and retrieve accurate transcriptions,
  ensuring that the transcription process is correctly integrated and functional.
...
What is tested:
- Testing if the service correctly orchestrates between the summary and LLM service and
  if the output is correctly structured and tracked.
...
- *MongoDB Collections*: Mocked using Moq to capture inserted documents and
  verify persistence without requiring a live database.

The use of callbacks captures documents during insertion,
allowing tests to verify both that the data was stored and that the content is correct.

=== Booking Backend - ASP.NET

The Booking Backend utilizes the same testing framework as the Consultation Backend: xUnit with Moq.
This consistency across backends enables shared testing patterns and
allows both services to be validated using identical mocking and isolation strategies.
...
There are many services used, such as Speech-to-Text or Email, that depend on external services and
are not suitable for unit testing. Migrations and auto-generated code also significantly contribute to the uncovered line count.

When looking at core logic, we get a much more representative result.
In the Consultation Backend the service layer achieves between 48.6% and 73.3%,
with the data models reaching 81.8% to 100% (see @consultation_test_coverage).
In the Booking Backend, the appointment and availability services both achieve 100% (see @booking_test_coverage).
...
=== LLM Service - Liquid AI #footnote[`docs/research/LLM-testing.typ`]

LLM model selection involved comparative benchmarking using two evaluation approaches.
The Summary and Suggestion quality assessment employed ROUGE-1 and BERTScore metrics to measure output accuracy and
semantic understanding while the MedQA benchmark assessed medical knowledge accuracy on a standardized dataset of 50 medical multiple-choice questions.
This dual-metric approach ensured the selected model (Liquid AI LFM2) balanced both clinical relevance and real-time performance requirements.

*Testing Strategy*
Rather than unit tests, this section uses benchmark metrics to assess output quality and
operational performance characteristics.
Two models were evaluated: Liquid AI (LFM2 2.6B parameters) and Gemma 4 (4B parameters).
...
What is tested:
- The ability to generate accurate clinical summaries from consultation transcripts using ROUGE-1 metric,
  which measures individual word matches and validates that key clinical terms are preserved.
- The model's contextual understanding of medical concepts using BERTScore F1 metric,
  which evaluates semantic similarity and ensures the generated summaries maintain clinical meaning.
- Latency and token throughput to ensure the model operates efficiently for real-time clinical use,
  with tests run on transcripts ranging from 1 to 14 minutes.

*Medical Knowledge Assessment (MedQA Benchmark)*

What is tested:
- The model's ability to correctly answer medical multiple-choice questions from the MedQA benchmark dataset,
  measuring medical knowledge accuracy.
...
=== Speech-to-text Service - Faster-Whisper #footnote[`docs/research/STT-test.typ`]

The Speech-to-Text service runs Faster-Whisper, an optimized implementation of OpenAI's Whisper model for speech recognition.
Performance validation employs custom benchmark tooling that measures throughput, latency, and
failure rates under varying concurrent loads.
This benchmarking approach was used to validate that the service meets our requirements,
where transcription must be completed within acceptable timeframes for doctor-patient consultations.

*Testing Strategy*

The Speech-to-Text Service validation employs stress testing and
benchmark analysis to evaluate performance and reliability under varying loads.
...
What is tested:
- The service's ability to process audio transcription requests at low concurrency
  (1-2 concurrent users) to establish baseline throughput and latency metrics.
- Performance degradation under gradually increasing concurrent load
  (1-16 concurrent requests) to identify where performance begins to degrade.
- Behavior under peak stress conditions (20-24 concurrent requests)
  to assess maximum capacity and failure rates.
- Recovery and stability after stress testing to ensure no permanent degradation from heavy load scenarios.

The tests use the same audio file across all phases.
The file is a mash-up of audio clips from Mozilla Common Voice dataset.
Concurrency levels represent simultaneous transcription requests.
Key metrics measured include throughput (requests/second),
mean latency, 95th percentile latency (p95), and failure rate.
...
- *Stable Throughput:*
  The service maintains approximately `0.42 requests/sec` regardless of concurrency level,
  indicating that request processing is sequential and request queuing,
  rather than parallelization, limits throughput.
- *Linear Latency Scaling:*
  Mean latency increases linearly with concurrency, which is expected when requests are queued.
  With stable throughput and proportional latency, the system remains predictable and reliable.
- *Zero Failure Rate:*
  No requests failed during any test phase, including peak stress conditions,
  demonstrating robust error handling and no resource exhaustion at the tested concurrency levels.
- *Full Recovery:*
  The recovery test shows the service returns to baseline performance,
  indicating no permanent degradation or resource leaks from sustained stress testing.
...
=== Auth0 for Authentication #footnote[`docs/research/authentication.typ`]

Authentication is provided by Auth0, a third-party identity platform with medical SSO compliance.
This choice eliminates the need for custom authentication testing, with
Auth0's certified compliance supporting HIPAA and other healthcare standards.

=== Mailpit for Email Testing #footnote[`docs/research/email.typ`]

Email functionality uses Mailpit for development and testing environments,
allowing complete control over email delivery without external dependencies.
Mailpit runs locally in Docker, enabling email integration testing without affecting external systems.

=== Typst for PDF Generation #footnote[`docs/research/pdf.typ`]

PDF prescriptions are generated using Typst, a text-based document system that allows data-driven PDF generation.
Typst's great customizability and data templating capabilities make it suitable for
clinical prescription documents with structured, reusable formatting.

=== Docker and Docker Compose

The entire system is containerized and orchestrated using Docker and Docker Compose,
enabling consistent testing environments across development, CI/CD, and deployment stages.
...
This semester's project successfully delivered a privacy-preserving OPD (Outpatient Department) management system that
meets the main objectives, defined in collaboration with the case owners at the beginning of the semester.
Our solution enables doctors to reduce mistakes made during consultations by reducing consultation administrative work,
while the system is secure in terms of handling sensitive patient data on the clinic's network.
Additionally, a proof of concept for the booking system was developed to demonstrate the end-to-end workflow
from patient booking to consultation, while keeping them separate.

Reflecting on the project, the following points were a success:
...
The system demonstrates the application of the knowledge gained throughout this semester's courses.
It demonstrates the design of a component-based system and the use of a locally-hosted large language model.
...
but also in differential diagnosis.

For the frontend part of our project, we did not spend so much time on making it a very accessible platform,
as our project is not a booking platform, it's an AI service.
However, we could extend it to add past consultation summaries and prescriptions after the fact,
appointment reminders via the email service and
a preconsultation form that the patient can fill out with symptoms to feed into the LLM context.

Furthermore, the speech-to-text (STT) component currently used in the system is not suited for horizontal scaling,
...
would make it one step closer to a real-world deployment.
