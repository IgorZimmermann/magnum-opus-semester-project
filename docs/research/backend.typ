...
On the other hand a monolithic (in our case two of them) approach in which the entire program is constructed as a single, indivisible unit. Any changes or updates or updates to the application require modifying and redploying the entire monolith. Monolithic architectures are often characterized by their simplicity and ease of development, especially for small to medium-sized applications. However, they can become complex and difficult to maintain as the size and complexity of the application grow. It is built as one large application with tightly coupled components. Scaling can be challenging but as our project's backend is only for handling the communication between the other microservices, its not a downside for us. The limitation that we will face (talked about later) is the technology stack's choice which is limited due to the structure. The last downside is that if one single part fails in the system the entire application goes down. The choice of having two backends tries to fix this. This way clients can still login and book appointments if the third party services fail.

Hono is a minimal TypeScript framework deisgned for edge runtimes and small APIs. It  intentionally gives freedom for the architecture convention. However, due to many people's missing knowledge in this field we will not choose this, no matter how much Scrum Mommy pushes it.

Java with spring boot is a used in many systems and proved itself. It has many libraries, including security and data access. However the team is inexperienced with this one as well and also developing for a such as short term this might not be the best choice.
...
1. Define two backend projects in a the repository
